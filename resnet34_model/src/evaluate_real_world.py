"""
Evaluates the already-trained proposed ResNet34 model on the held-out
real-world field photo test set (data/real_environment_test/), separate
...
def main():
    ...
    report_path = os.path.join(evaluation_report_dir, "classification_report_real_world.txt")
    with open(report_path, "w") as f:
        f.write("Tomato Leaf Disease Detection - Real-World Test Set Evaluation Report\n")
        f.write("=" * 60 + "\n\n")
        f.write(report_text)
    ...
    results_path = os.path.join(evaluation_report_dir, "eval_results_real_world.json")
    ...
    plt.title('Confusion Matrix - Real-World Test Set (ResNet34 proposed)')
    plt.ylabel('True Disease')
    plt.xlabel('Predicted Disease')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()

    cm_path = os.path.join(evaluation_report_dir, "cm_real_world_test.png")
